...
class JKOSolver(tf.keras.models.Model):
    ...
    def prepare(self):
        ...
        self.prev_weights = tf.reshape(self.prob(self.prev_ref_pts), (-1,))
        ...
        self.importance_density = self.prob(self.curr_ref_pts)
    ...
    def update(self, domain, epochs=100, initial_rate=1e-3, nitn=10, neval=200):
        ...
        print(self.current_time, self.curr_ref_pts, self.prev_ref_pts)
        for epoch in range(epochs):
            with tf.GradientTape() as tape:
                ...
                loss_F = self.loss_F()
                loss = 0.5 * loss_W + self.time_step * loss_F
    ...
    def loss_F(self):
        ...
        """
        rho = self.prob(self.curr_ref_pts)
        return tf.reduce_mean((self.psi(self.curr_ref_pts) + tf.math.log(rho) / self.beta) * rho / self.importance_density)
    ...
    def learn_density(self, ensemble, weights, domain, epochs=100, initial_rate=1e-3, nitn=10, neval=200):
        ...
        print('MC integral 1: {}'.format(tf.reduce_mean(tf.divide(self.prob(samples),  imp))))
        self.normalizer = tf.reduce_mean(tf.divide(self.prob(ensemble),  weights_))
    ...
    def compute_normalizer(self, domain, nitn=10, neval=200):
        ...
        def integrand(x, n_dim=None, weight=None):
            return self.prob(tf.convert_to_tensor([x], dtype=self.dtype)).numpy()[0][0]
    ...
    def save_weights(self, time_id=None):
        ...
        hdf5.create_array(hdf5.root, 'time_' + str(self.current_time), np.array([self.normalizer]))
        hdf5.close()
    ...
    def load_weights(self, time_id):
        ...
        self.normalizer = getattr(hdf5.root, 'time_' + str(self.current_time)).read()[0]
        hdf5.close()
